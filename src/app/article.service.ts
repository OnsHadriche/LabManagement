...
export class ArticleService {
  ...
  ONSAVE(articleTosave: any): Observable<any> {
    ...
    // return this.httpClient.post('127.0.0.1:8080/api/articles',articleTosave)
    const Article1 = {
      ...articleTosave,
  ...
  updateArticle(id: string, form: any): Observable<any> {
    return this.httpClient.put<void>(`http://localhost:3000/articles/${id}`,form);
  ...
  getArticleById(id: string): Observable<Article> {
    ...
    //   observer.next(this.tab1.filter((item) => item.id == id)[0] ?? null)
    // );
  }
  onsave(from: any): Observable<void> {
    return this.httpClient.post<void>('localhost:3000/articles', from);
  }
}
